...
public final class StringMetricBuilderExample {
	 ...
	 * adding simplifiers, tokenizers and filters and transform the effectiveness of
	 * a metric can be improved.
	 * 
	 * The exact combination is generally domain specific. The StringMetricBuilder
	 * supports these domain specific customizations. Some example usages are shown
	 * below
	 */
	 ...
	 * string to a simpler format. This allows string from different sources to be
	...
	public static float example02() {
		...
		StringMetric metric = with(new Levenshtein()).simplify(Simplifiers.removeDiacritics())
	 ...
	 * Tokenization
	 * 
	 * A metric can be used to measure the similarity between strings. However not
	 * all metrics can operate on strings directly. Some operate on lists, sets or
	 * multisets. To compare strings with a metric that works on a collection a
	 * tokenizer is required. Tokenization cuts up a string into parts.
	 * 
	 * Example:
	 * 
	 ...
	 * `[chilperic, ii, son, of, childeric, ii]`
	 * 
	 * The choice of the tokenizer can influence the effectiveness of a metric. For
	 * example when comparing individual words a q-gram tokenizer will be more
	 * effective while a whitespace tokenizer will be more effective when comparing
	 * documents.
	 * 
	 * The Tokenizers utility class contains a collection of common, useful
	 * tokenizers. For a custom tokenizer you can implement the Tokenizer interface.
	 * Though it is recommended that you extend the AbstractTokenizer.
	 */
	...
	public static float example04() {
		...
		StringMetric metric = with(new CosineSimilarity<String>()).tokenize(Tokenizers.whitespace())
	...
	/**
	 * Tokens can be filtered to avoid comparing strings on common but otherwise low
	 * information words. Tokens can be filtered after any tokenization step and
	 * filters can be applied repeatedly.
	 * 
	...
	public static float example05() {
		...
		StringMetric metric = with(new CosineSimilarity<String>()).simplify(Simplifiers.toLowerCase())
	...
	/**
	 * Tokens can be transformed to a simpler form. This may be used to reduce the
	 * possible token space. Tokens can be transformed after any tokenization step
	 * and the transformation can be applied repeatedly.
	 * 
	 * A transformation can be implemented by implementing a the Function interface.
	 */
